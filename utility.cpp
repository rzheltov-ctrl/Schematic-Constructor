...
int PCB_lay ( int fp_lay )
{
	if( fp_lay == LAY_PART_NAME || fp_lay == LAY_PIN_LINE )
		return fp_lay;
	else if( fp_lay == LAY_FP_SILK_TOP )
		return LAY_PIN_DESC;
	else if( fp_lay == LAY_FP_SILK_BOTTOM )
		return LAY_CONNECTION;
	else if( fp_lay == LAY_FP_TOP_COPPER )
		return LAY_ADD_1;
	else if( fp_lay == LAY_FP_BOTTOM_COPPER )
		return LAY_ADD_2;
	else if( fp_lay == LAY_FP_INNER_COPPER )
		return LAY_ADD_2+1;
	else
		return fp_lay;
}
...
int Get_fp_Side ( int fp_lay )
{
	if( fp_lay == LAY_PART_NAME || 
		fp_lay == LAY_FP_SILK_TOP ||
		fp_lay == LAY_FP_TOP_COPPER )
		return 0;
	else if( fp_lay == LAY_PIN_LINE || 
		fp_lay == LAY_FP_SILK_BOTTOM ||
		fp_lay == LAY_FP_BOTTOM_COPPER )
		return 1;
	else
		return 2;
}
...
int FindArcElements ( CPolyLine * p, int * i, BOOL * EndFlag )
{
	...
	*EndFlag = FALSE;
	...
	while(1)
	{
		...
		{
			pv.x = p->GetX( p->GetPrevCornerIndex(ii) );
			pv.y = p->GetY( p->GetPrevCornerIndex(ii) );
			v.x = p->GetX( ii );
			v.y = p->GetY( ii );
			nv.x = p->GetX( p->GetNextCornerIndex(ii) );
			nv.y = p->GetY( p->GetNextCornerIndex(ii) );
...
int RemoveColinearSegments( CArray<CPolyLine> * arr )
{
	int nDel = 0;
	for( int i=0; i<arr->GetSize(); i++ )
	{
		if( arr->GetAt(i).GetSel() == 0 )
			continue;
		int cl = arr->GetAt(i).GetClosed();
		for( int ii=cl?0:1; ii<(cl?arr->GetAt(i).GetNumCorners():arr->GetAt(i).GetNumSides()); ii++ )
		{
			int iprev = arr->GetAt(i).GetPrevCornerIndex(ii);
			int inext = arr->GetAt(i).GetNextCornerIndex(ii);
			int x1 = arr->GetAt(i).GetX(iprev);
			int y1 = arr->GetAt(i).GetY(iprev);
			int x2 = arr->GetAt(i).GetX(ii);
			int y2 = arr->GetAt(i).GetY(ii);
			int x3 = arr->GetAt(i).GetX(inext);
			int y3 = arr->GetAt(i).GetY(inext);
			int ss1 = arr->GetAt(i).GetSideStyle(iprev);
			int ss2 = arr->GetAt(i).GetSideStyle(ii);
			if (ss1 || ss2)
				continue;
			if( Colinear( x1,y1,x2,y2,x3,y3 ) )
			{
				if( arr->GetAt(i).GetNumCorners() > 2 )
				{
					arr->GetAt(i).DeleteCorner( ii );
					nDel++;
					ii--;
				}
			}
			else if((abs(x1-x2) < _2540 && abs(y1-y2) < _2540 ) ||
				( abs(x3-x2) < _2540 && abs(y3-y2) < _2540 ))
			{
				if( arr->GetAt(i).GetNumCorners() > 2 )
				{
					arr->GetAt(i).DeleteCorner( ii );
...
int Generate_Dotted (int xi, int yi, int xf, int yf, CPoint * OutPut, int n_p )
{
	float stepX = (float)(xf - xi)/(float)(n_p-1);
	float stepY = (float)(yf - yi)/(float)(n_p-1);
	for( int i=0; i<n_p; i++ )
	{
		OutPut[i].x = xi + stepX*i;
		OutPut[i].y = yi + stepY*i;
	}
	return n_p;
}
...
int SwellPolygon( CPoint * P, int np, CPoint * newP, int swValue )
{
	...
	for( int i=0; i<np; i++ )
	{
		...
		else
		{
			...
			CPoint * pARC = new CPoint[n_sides];//new014
			if( pDir == dCW )
			{
...
int TestPolygon( int X, int Y, CPoint * P, int np, int * distance )
{
	enum{
		dir___CW=1,
		dir___CCW
	...
	for (int q=0; q<np; q++)
    {
		...
		{
			int gx=0, gy=0;
			int dd = ::GetClearanceBetweenSegments( P[q].x, P[q].y, P[q-1].x, P[q-1].y, 0, 1,
														X, Y, X+10, Y+10, 0, 1, NM_PER_MM*10, &gx, &gy );
			if( dd < *distance )
			{
				RET = 1;
				*distance = dd;
	...
	{
		int gx=0, gy=0;
		int dd = ::GetClearanceBetweenSegments( P[0].x, P[0].y, P[np].x, P[np].y, 0, 1,
													X, Y, X+10, Y+10, 0, 1, NM_PER_MM*10, &gx, &gy );
		if( dd < *distance )
		{
			RET = 1;
...
//
void MakeCStringFromDimension( double scale, CString * str, int dim, int units, BOOL append_units,
							  BOOL lower_case, BOOL space, int max_dp, BOOL strip )
{
	...
	f_str = "%" + f_str;
	if( units == MM )
		str->Format( f_str, (double)dim/(double)NM_PER_MM*(double)scale );
	else if( units == MIL )
		str->Format( f_str, (double)dim/(double)NM_PER_MIL*(double)scale );
	else if( units == NM )
		str->Format( "%d", (int)((double)dim*(double)scale) );
	else
		ASSERT(0);
	str->Trim();
...
int ParseRef( CString * ref, CString * prefix, CString * suffix )
{
	// get length of numeric suffix
	int ref_length = ref->GetLength();
	int pref_length = 0;
	...
	if( pref_length == 0 )
		*prefix = "";
	else if( pref_length == ref_length )
	{
		*prefix = *ref;
		*suffix = "";
		return 0;
		...
		*prefix = ref->Left( pref_length );
	CString str = ref->Right( ref_length - pref_length );
	...
	CString num_str = str.Left( num_length );
	int num = atoi( num_str );
	if( num_length == str_length )
	{
		*suffix = "";
		return num;
	}
	*suffix = str.Right( str_length - num_length );
	return num;
}
//
int GetRightNumber( CString * ref, CString * prefix, CString * postfix, int * num_Len )
{
	int S_index = 0, E_index = -1;
	int i=ref->GetLength()-1;
	for( ; i>=0; i-- )
		if( ref->GetAt(i) >= '0' && ref->GetAt(i) <= '9' )
		{
			E_index = i;
			break;
		}
	...
	*prefix = ref->Left( S_index );
	*postfix = ref->Right( ref->GetLength() - E_index - 1 );
	*num_Len = (E_index - S_index + 1);
	if( *num_Len )
	{
		CString str = ref->Left( E_index + 1 );
		str = str.Right( *num_Len );
		return my_atoi( &str );
	}
	return 0;
}
...
BOOL CheckLegalPinName( CString * pinstr, CString * astr, CString * nstr, int * n )
{
	...
	if( (*pinstr).GetLength() == 0 )
		return FALSE;
	if( -1 != pinstr->FindOneOf( ILLEGAL_PIN ) )
		return FALSE;
								   ...
								   int max_cl, int * x, int * y )
{
	...
	if( el1.theta2 > el1.theta1 )
		el1.theta2 = el1.theta1;
	if( bArcs && el2.theta2 > el2.theta1 )
		el2.theta2 = el2.theta1;
...
//
double Distance( int x1, int y1, int x2, int y2, BOOL bARC )
{
	double d=0;
	if( bARC )
	{
		CPoint PO[N_SIDES_APPROX_ARC*2+1];
		int n_sides = Generate_Arc( x1,y1,x2,y2, 1, PO, N_SIDES_APPROX_ARC*2 );
...
float ex_float( CString * f )
{
	...
	CString s = f->Right( length - pref_length );
	s.Replace( ",","." );
	pref_length = 0;
	for( int i=0; i<length; i++ )
	{
	...
	s = s.Left( pref_length );
	return my_atof( &s );
}

CString GenNextSuffix( CString * old )
{
	CString SORT = CSORT;
	int f = SORT.Find( old->Right(1) );
	if( f >= 0 )
		return (old->Left( old->GetLength()-1 ) + SORT[f+1]);
	else
		return "A";
}

void ExtractComponentName( CString * Value, CString * Footprint )
{
	if( Value )
	{
		for( int i=Value->FindOneOf( ILLEGAL_FILENAME ); i>=0; i=Value->FindOneOf( ILLEGAL_FILENAME ) )
			Value->Delete(i);
		*Value = Value->Trim();
	}
	if( Footprint )
	{
		for( int i=Footprint->FindOneOf( ILLEGAL_FILENAME ); i>=0; i=Footprint->FindOneOf( ILLEGAL_FILENAME ) )
			Footprint->Delete(i);	
		*Footprint = Footprint->Trim();
	}
}

CText * GetPartPin( CArray<CPolyLine> * poly_arr, CText * partT, CString * pinN )
{
	for( int get=partT->m_polyline_start; get>=0; get=poly_arr->GetAt(get).Next[index_part_attr] )
	{
		if( poly_arr->GetAt(get).pAttr[index_pin_attr] )
			if( pinN->Compare( poly_arr->GetAt(get).pAttr[index_pin_attr]->m_str ) == 0 )
				return poly_arr->GetAt(get).pAttr[index_pin_attr];
	}	
	return NULL;
}
...
{
	int wlen = MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, url->GetBuffer(), -1, NULL, 0 );
	wchar_t * wurl = new wchar_t[wlen];
	MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, url->GetBuffer(), -1, wurl, wlen );
	int alen = WideCharToMultiByte( CP_UTF8, 0, wurl, -1, NULL, 0, 0, 0 );
	char * aurl = new char[alen];
	WideCharToMultiByte( CP_UTF8, 0, wurl, -1, aurl, alen, 0, 0 );
	aurl[alen-1] = '\0';
	CString ret = aurl;
	delete wurl;
	delete aurl;
